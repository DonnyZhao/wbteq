...
def get_all_steps():
    # TODO: to be implemented with db connection
    s1 = Step(1,1,'j1_s1.bteq',20)
    s2 = Step(1,2,'j1_s2.bteq',10)
...
def command_line_runner():
    ...
    for cmd_email in command_files:
        ...
        if args['exec'] is False:
            logger.info('Command to be run: {}'.format(cmd))
        else:
            # rcode = call([cmd])
            rcode = 0
            if rcode == 0:
            ...
            logger.info('Sending email for {} to {}'.format(cmd, email))
            send_notification(cmd, email)
